...
def policy_iteration(model: Model, n_episodes: int = 100):
    ...
    for i in tqdm(range(n_episodes)):
        for _ in range(5): # set 5 iterations
            policy_evaluation()
        pi_old = np.copy(pi)
        policy_improvement()
        diffs = sum((pi_old != pi))
        diff_per_episode.append(diffs) # tracking how many policy changes occurred in each episode (sequence of interactions between the agent and the environment) of the iteration process.
        if all(pi_old == pi):
            break
